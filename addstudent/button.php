    ...
    <div class="modal fade" id="del<?php echo $row['userid']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
				...
				<?php
					$del=mysqli_query($conn,"select * from login where userid='".$row['userid']."'");
					$drow=mysqli_fetch_array($del);
				?>
				<div class="container-fluid">
					<h5><center>Are you sure to delete <strong><?php echo ucwords($drow['username'].' '.$row['password']); ?></strong> from the list? This method cannot be undone.</center></h5> 
                </div> 
				</div>
    ...
    <div class="modal fade" id="edit<?php echo $row['userid']; ?>" tabindex="5" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
				<?php
					$edit=mysqli_query($conn,"select * from login where userid='".$row['userid']."'");
				...
				<form method="POST" action="edit.php?id=<?php echo $erow['userid']; ?>">
					...
					<div class="row">
						<div class="col-lg-2">
							<label style="position:relative; top:7px;">Usertype:</label>
						</div>
						<div class="col-lg-10">
							<input type="text" name="usertype" class="form-control" value="<?php echo $erow['usertype']; ?>">
						</div>
					</div>
                </div> 
				</div>
